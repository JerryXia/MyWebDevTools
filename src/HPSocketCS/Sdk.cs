...
namespace HPSocketCS.SDK
{
    ...
    {
        static bool Is64OperatingSystem = false;
        static HPSocketSdk()
        {
            string dllDir = AppDomain.CurrentDomain.BaseDirectory.TrimEnd(new char[] { Path.DirectorySeparatorChar });
            string filePath = Path.Combine(dllDir, SOCKET_DLL_PATH);

            GetSystemProcessorInfo_WMI();
            // NET40
            //Environment.Is64BitOperatingSystem
            ReleaseFile(filePath, Is64OperatingSystem ? LibResource.HPSocket4C_U_x64 : LibResource.HPSocket4C_U_x86);
        }
        static void GetSystemProcessorInfo_WMI()
        {
            var dSystemManangement = System.Reflection.Assembly.Load("System.Management, Version=2.0.50727.5483, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a");
            ...
            var ctor = tManagementObjectSearcher.GetConstructor(new System.Type[] { typeof(string) });
            var mGet = tManagementObjectSearcher.GetMethod("Get", new System.Type[] { });
            ...
            foreach (object processor in processors as System.Collections.IEnumerable)
            {
                try
                {
                    System.Reflection.PropertyInfo prop = tManagementObject.GetProperty("Item", new System.Type[] { typeof(string) });
                    ushort bit = (ushort)prop.GetValue(processor, new object[] { "AddressWidth" });
                    HPSocketSdk.Is64OperatingSystem = (bit == 64);
                }
                catch
                {
                }
        ...
        static void ReleaseFile(string filePath, byte[] content)
        {
            if (File.Exists(filePath))
            {
                ...
                {
                    isEquals = (fileBytes[i] == content[i]);
                    if (!isEquals)
                    {
                        break;
                    }
                ...
                if (!isEquals)
                {
                    WriteByteFile(filePath, content);
                }
            }
            else
            {
        ...
        static void WriteByteFile(string filePath, byte[] content)
        {
